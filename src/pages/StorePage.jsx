...
function Lightbox({ images, startIdx, onClose }) {
  ...
  return (
    <div className="lb-overlay" onClick={onClose}>
      <div className="lb-shell" onClick={e => e.stopPropagation()} onMouseDown={onDown} onMouseUp={onUp} onTouchStart={onDown} onTouchEnd={onUp}>
        ...
        <div className="lb-stage">
          ...
          <img key={idx} src={images[idx]} alt={`Image ${idx + 1}`}
...
function CardCarousel({ images, serviceName, onImageClick }) {
  ...
  if (!images.length) return <div className="card-img-fallback"><span>{serviceName[0]}</span></div>;
      ...
      <div className="carousel-strip" onClick={() => onImageClick(idx)}>
        {images.map((url, i) => (
          <img key={i} src={url} alt={`${serviceName} ${i+1}`}
            className={`carousel-slide ${i===idx?'carousel-active':''}`} draggable={false}
            onError={e => { e.currentTarget.style.display='none'; }}/>
        ))}
        <div className="carousel-zoom-hint"><ZoomIn size={13}/> {t('store.view')}</div>
      </div>
      {images.length > 1 && (
        <>
  ...
  return (
    <div className="svc-card-grid">
      <div className="svc-card-img-wrap">
        ...
        {saved > 0 && <div className="svc-sale-badge">{t('store.sale')}</div>}
      </div>
      <div className="svc-card-body">
        <h3 className="svc-card-name">{service.name}</h3>
        {service.description && <p className="svc-card-desc">{service.description}</p>}
        <div className="svc-card-meta">
          <span><Clock size={11}/> {service.duration}{t('common.min')}</span>
          {service.rating?.count > 0 && (
            <span className="svc-rating-pill"><Star size={11} fill="currentColor"/> {service.rating.average?.toFixed(1)}</span>
          )}
        ...
        <div className="svc-card-footer">
          ...
          <Link to={`/book/${service._id}`} className="svc-book-btn">{t('store.book')} <ChevronRight size={12}/></Link>
        </div>
      </div>
    </div>
  );
...
function ServiceRowList({ service, onOpenLightbox }) {
  ...
  return (
    <div className="svc-row-list">
      <div className="svc-row-thumb" onClick={() => images.length > 0 && onOpenLightbox(images, 0)} style={{ cursor: images.length > 0 ? 'zoom-in' : 'default' }}>
        {images[0] ? <>
          <img src={images[0]} alt={service.name} onError={e => { e.currentTarget.style.display='none'; }}/>
          <div className="carousel-zoom-hint"><ZoomIn size={12}/> {t('store.view')}</div>
      ...
      <div className="svc-row-content">
        ...
        <div className="svc-row-footer">
          <div className="svc-row-meta">
            <span><Clock size={12}/> {service.duration} {t('common.minutes')}</span>
            {service.rating?.count > 0 && (
              <span className="svc-rating-pill"><Star size={11} fill="currentColor"/> {service.rating.average?.toFixed(1)}<span className="rating-count">({service.rating.count})</span></span>
            )}
            {service.totalBookings > 0 && <span><CheckCircle2 size={12}/> {service.totalBookings} {t('common.booked')}</span>}
          </div>
          <Link to={`/book/${service._id}`} className="svc-book-btn">{t('store.bookNow')} <ChevronRight size={13}/></Link>
        </div>
      </div>
    </div>
  );
    ...
    <div className="sort-dropdown" ref={ref}>
      ...
      {open && (
        <div className="sort-menu">
          <div className="sort-menu-header">{t('common.sortBy')}</div>
          {SORT_OPTIONS.map(opt => (
            <button key={opt.value} className={`sort-item ${value === opt.value ? 'active' : ''}`}
...
export default function StorePage() {
  ...
  const filtered = useMemo(() => {
    ...
    if (search.trim()) {
      ...
      list = list.filter(s =>
        ...
        s.category?.toLowerCase().includes(q) || s.tags?.some(t => t.toLowerCase().includes(q))
      );
    }
  ...
  if (loading) return (
    <div className="sp-loading"><div className="sp-spinner"/><p>{t('store.loading')}</p></div>
  );
  if (!store) return (
    <div className="sp-not-found">
      ...
      <Link to="/" className="sp-nf-btn">{t('common.goHome')}</Link>
    </div>
  );

  const tabs = [
    { id: 'services', label: t('store.servicesTab'), count: services.length },
    { id: 'reviews',  label: t('store.reviewsTab'),  count: reviews.length  },
    { id: 'hours',    label: t('store.hoursTab'),     count: null            },
  ];

  return (
    <div className="sp-root" data-theme={store.theme || 'classic'}>
      <div className="sp-banner">
        {store.banner ? <img src={store.banner} alt={store.name} className="sp-banner-img"/> : <div className="sp-banner-fallback"/>}
      ...
      <div className="sp-header-wrap">
        <div className="sp-header-inner">
          <div className="sp-logo-wrap">
            {store.logo ? <img src={store.logo} alt={store.name} className="sp-logo"/> : <div className="sp-logo-fallback">{store.name[0]}</div>}
          </div>
          <div className="sp-header-body">
            <div className="sp-header-left">
              {store.category && <div className="sp-category-pill"><Award size={11}/> {store.category}</div>}
              ...
              <div className="sp-store-stats">
                {avgRating && (
                  <div className="sp-stat-pill sp-stat-rating">
                    <Star size={12} fill="currentColor"/>{avgRating}<span>({reviews.length})</span>
                  </div>
                )}
                {store.address?.city && (
                ...
                <div className="sp-stat-pill"><Calendar size={12}/> {services.length} {t('store.services')}</div>
              </div>
            </div>
            <div className="sp-header-right">
              {store.phone && <a href={`tel:${store.phone}`} className="sp-contact-btn"><Phone size={14}/> <span>{store.phone}</span></a>}
              {store.email && <a href={`mailto:${store.email}`} className="sp-contact-btn"><Mail size={14}/> <span>{store.email}</span></a>}
              {store.website && <a href={store.website} target="_blank" rel="noopener noreferrer" className="sp-contact-btn"><Globe size={14}/> <span>{t('common.website')}</span> <ExternalLink size={12}/></a>}
                ...
                <div className="sp-social-row">
                  {store.socialLinks?.instagram && (
                    <a href={store.socialLinks.instagram} target="_blank" rel="noopener noreferrer" className="sp-social-btn" title="Instagram">
                      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="2" y="2" width="20" height="20" rx="5"/><circle cx="12" cy="12" r="4"/><circle cx="17" cy="7" r="1"/></svg>
                    </a>
                  )}
                  ...
                  {store.socialLinks?.twitter && (
                    <a href={store.socialLinks.twitter} target="_blank" rel="noopener noreferrer" className="sp-social-btn" title="Twitter">
                      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M4 4l16 16M20 4L4 20"/></svg>
                    </a>
                  )}
                </div>
              )}
      ...
      <div className="sp-tabs-bar">
        <div className="sp-tabs-inner">
          {tabs.map(tab => (
            <button key={tab.id} className={`sp-tab ${activeTab === tab.id ? 'sp-tab-active' : ''}`} onClick={() => setActiveTab(tab.id)}>
              {tab.label}
              {tab.count !== null && <span className={`sp-tab-count ${activeTab === tab.id ? 'sp-tab-count-active' : ''}`}>{tab.count}</span>}
            </button>
          ))}
        </div>
      </div>
          ...
          <div className="sp-services-tab">
            <div className="sp-controls">
              <div className="sp-search-wrap">
                ...
                <input ref={searchRef} className="sp-search" placeholder={t('store.searchPlaceholder')} value={search} onChange={e => setSearch(e.target.value)}/>
                {search && <button className="sp-search-clear" onClick={() => { setSearch(''); searchRef.current?.focus(); }}><X size={14}/></button>}
              </div>
            ...
            {categories.length > 0 && (
              <div className="sp-category-scroll">
                <button className={`sp-cat-chip ${!category?'active':''}`} onClick={() => setCategory('')}>
                  {t('common.all')} <span>{services.length}</span>
                </button>
                {categories.map(c => {
            ...
            {(search || category) && (
              <div className="sp-result-info">
                {filtered.length} {filtered.length !== 1 ? t('common.results_plural') : t('common.results')}
                {search && <> for "<strong>{search}</strong>"</>}
                {category && <> in <strong>{category}</strong></>}
                <button className="sp-clear-filters" onClick={() => { setSearch(''); setCategory(''); }}>{t('store.clearFilters')}</button>
              </div>
            )}
            ...
            {filtered.length === 0 ? (
              <div className="sp-empty">
                ...
                <h3>{t('store.noServicesFound')}</h3>
                <p>{search || category ? t('store.trySearch') : t('store.noServicesDesc')}</p>
                {(search || category) && <button className="sp-nf-btn" onClick={() => { setSearch(''); setCategory(''); }}>{t('store.clearFilters')}</button>}
              </div>
            ) : layout === 'grid' ? (
              <div className="sp-grid">{filtered.map(s => <ServiceCardGrid key={s._id} service={s} onOpenLightbox={openLightbox}/>)}</div>
            ) : (
              <div className="sp-list">{filtered.map(s => <ServiceRowList key={s._id} service={s} onOpenLightbox={openLightbox}/>)}</div>
            )}
          </div>
        )}
        ...
        {activeTab === 'reviews' && (
          <div className="sp-reviews-tab">
            {reviews.length === 0 ? (
              <div className="sp-empty">
                <div className="sp-empty-icon">💬</div>
                <h3>{t('store.noReviews')}</h3>
                <p>{t('store.noReviewsDesc')}</p>
              </div>
            ) : (
              <>
                <div className="rv-summary">
                  <div className="rv-summary-score">
                    ...
                    <div className="rv-stars">
                      {[1,2,3,4,5].map(i => <Star key={i} size={18} fill={i<=Math.round(Number(avgRating))?'#f59e0b':'none'} stroke={i<=Math.round(Number(avgRating))?'#f59e0b':'#d1d5db'}/>)}
                    </div>
                    <div className="rv-total">{reviews.length} {t('store.reviews')}</div>
                  </div>
        ...
        {activeTab === 'hours' && (
          <div className="sp-hours-tab">
            <div className="hours-card">
              <div className="hours-card-title"><Clock size={15}/> {t('store.businessHours')}</div>
                    ...
                    return (
                      <div key={day} className={`hours-row ${isToday?'hours-today':''}`}>
                        <div className="hours-day-wrap">
                          ...
                          {isToday && <span className="today-pill">{t('store.today')}</span>}
                        </div>
                        {h?.isOpen
                          ? <span className="hours-time"><Clock size={12}/> {h.openTime} – {h.closeTime}</span>
                          : <span className="hours-closed">{t('common.closed')}</span>
                        }
                      </div>
                    );
                  })}
                </div>
              ) : <p className="hours-empty">{t('store.hoursNotSet')}</p>}
            </div>
          </div>
        )}
      </div>
